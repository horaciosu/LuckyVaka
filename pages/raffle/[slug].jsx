          ...
          <div style={{ position: 'sticky', top: 80 }}>
            ...
            <div className="card" style={{ textAlign: 'center', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 12 }}>
                {[[countdown.d, t('days', 'días')], [countdown.h, 'hrs'], [countdown.m, 'min'], [countdown.s, 'seg']].map(([val, label], i) => (
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {[1, 3, 5, 10, 20].map(n => (
                  <button key={n} onClick={() => setQty(n)} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, cursor: 'pointer', border: '1px solid var(--border)', background: qty === n ? 'var(--brand)' : 'transparent', color: qty === n ? '#fff' : 'var(--muted)' }}>x{n}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8, display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontWeight: 500 }}>{t('Pick your numbers', 'Elige tus números')}</span>
                <span style={{ color: 'var(--brand)', fontSize: 10, fontWeight: 600 }}>{selected.size}/{qty}</span>
              </div>
            ...
            <div style={{ background: '#F4F3EF', borderRadius: 8, padding: '10px 12px', marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--muted)', marginBottom: 4 }}>
                <span>{qty} x {raffle.ticket_price} {raffle.currency}</span><span>{subtotal.toFixed(2)}</span>
              </div>
            ...
            <div style={{ fontSize: 10, color: 'var(--muted)', textAlign: 'center', lineHeight: 1.5 }}>
              ✓ {t('Guaranteed winner · Full refund if not funded', 'Ganador garantizado · Reembolso si no se activa')}
            </div>
          </div>
        </div>
      </div>
